...
export class ActivityComponent implements OnInit {
  ...
  //Chart selection
  charts: Record<string, boolean> = {"line":true, "histogram":false};
  ...
  displayData: {name:string, series:{name:string, value:number}[]} []= [];
  ...
  updateChart() {
    ...
    this.displayData = []
    this.dataset = this.activityService.getGraphData(this.selectedMetric, this.activity, this.xAxisRepresents)
    this.displayData.push({'name':this.selectedMetric, 'series':this.dataset})
    this.pushExtras()
    this.updateRatings()
  }
  pushExtras(){
