    ...
    return next.handle(req).pipe(
      catchError(err => {
        if (err.status === 401) {
          ...
          this.alertService.alert("You are not logged in, or your credentials have expired. \n If you think this is a mistake, you can reload the page. If it was a mistake you'll be allowed to continue.\n Otherwise, click 'ok', and you'll be redirected to the login page.", "Not logged in",  ()=>{this.authService.logout();this.router.navigate(['/'])});
